// Las siete herramientas de surcodia-payments, registradas sobre un
// McpServer nuevo. Lo usan las dos entradas: server.ts (stdio, para Claude
// Code y Grok Build en la máquina local) y src/app/api/mcp/route.ts (HTTP,
// para clientes en la nube como Grok Bot). Un solo lugar define qué
// herramientas hay y qué dicen de sí mismas.
...
export function createPaymentsServer() {
  const server = new McpServer({ name: "surcodia-payments", version: "1.1.0" });
